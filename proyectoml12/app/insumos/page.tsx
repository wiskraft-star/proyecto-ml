...
type SupplyRow = Prisma.SupplyGetPayload<{}>;
...
export default async function InsumosPage(): Promise<JSX.Element> {
  const supplies: SupplyRow[] = await prisma.supply.findMany({ orderBy: { name: "asc" } });
  const recipeLines: RecipeLineWithSupply[] = await prisma.supplyRecipeLine.findMany({
    include: { supply: true },
