...
import type { Prisma, Payment as MpPayment, SkuCost } from "@prisma/client";

type SaleWithItems = Prisma.SaleGetPayload<{ include: { items: true } }>;
type RecipeLineWithSupply = Prisma.SupplyRecipeLineGetPayload<{ include: { supply: true } }>;
...
export default async function MetricasPage({
  ...
  const payments: MpPayment[] = await prisma.payment.findMany({
    where: { paidAt: { gte: from, lt: to } },
    orderBy: { paidAt: "desc" },
    take: 2000,
  });

  const skuCostsArr: SkuCost[] = await prisma.skuCost.findMany();
  const skuCosts = new Map(skuCostsArr.map((c) => [c.sku, c]));

  const recipeLines: RecipeLineWithSupply[] = await prisma.supplyRecipeLine.findMany({ include: { supply: true } });
