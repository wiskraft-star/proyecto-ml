...
function titleFromPath(pathname: string | null) {
  ...
  if (pathname.startsWith("/margen")) {
    const seg = pathname.split("/").filter(Boolean)[1] ?? "metricas";
    const map: Record<string, string> = {
      ventas: "Ventas (MVP)",
      cobros: "Cobros / Liquidaciones (MVP)",
      cogs: "Stock / Costos (MVP)",
      insumos: "Insumos (MVP)",
      "mano-obra": "Mano de obra (MVP)",
      metricas: "Métricas (MVP)",
    };
    return map[seg] ?? "Margen neto (MVP)";
  }
  ...
  if (pathname.startsWith("/reportes")) return "Métricas";
