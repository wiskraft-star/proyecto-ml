...
export default function DashboardPage() {
  ...
  const [compare, setCompare] = useState<"none" | "prev" | "year" | "custom">("prev");
  const [from, setFrom] = useState("2025-12-01");
  const [to, setTo] = useState("2025-12-13");
  const [compareFrom, setCompareFrom] = useState("2025-11-24");
  const [compareTo, setCompareTo] = useState("2025-11-30");
  const [customOpen, setCustomOpen] = useState(false);
  const [customTarget, setCustomTarget] = useState<"primary" | "compare">("primary");
  ...
  const compareLabel = useMemo(() => {
    if (compare === "none") return "Sin comparación";
    if (compare === "prev") return "Período anterior";
    if (compare === "year") return "Año anterior";
    return `${compareFrom} → ${compareTo}`;
  }, [compare, compareFrom, compareTo]);

  function onPrimaryChange(value: string) {
    // UI tipo ML: presets + opción “Fecha personalizada” que abre un modal
    if (value === "custom") {
  ...
  function onCompareChange(value: string) {
    if (value === "custom") {
      setCustomTarget("compare");
      setDraftFrom(compareFrom);
      setDraftTo(compareTo);
      setCustomOpen(true);
      return;
    }
    setCompare(value as any);
  }
  ...
  function applyCustomRange() {
    if (customTarget === "primary") {
      ...
      setCompareFrom(draftFrom);
      setCompareTo(draftTo);
      setCompare("custom");
    }
    setCustomOpen(false);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Período (barra compacta, referencia: ML) */}
      <div className="flex flex-wrap items-end gap-3 rounded-2xl border border-border bg-panel/60 p-3">
        <div className="min-w-[190px]">
          <div className="text-[11px] font-semibold text-muted">Período principal</div>
          <Select
            className="mt-1 h-9 py-1.5"
            value={preset === "rango" ? "custom" : preset}
        ...
        <div className="min-w-[190px]">
          <div className="text-[11px] font-semibold text-muted">Comparar con</div>
          <Select className="mt-1 h-9 py-1.5" value={compare} onChange={(e) => onCompareChange(e.target.value)}>
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={() => {
              ...
              alert("Filtros aplicados (modo demo). En fase 2, esto recalculará con datos reales.");
            }}
          >
            Filtrar
          </Button>
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <Badge tone="neutral">{compareLabel}</Badge>
        </div>
      </div>
      ...
      <Modal
        ...
        title={customTarget === "primary" ? "Período principal: fecha personalizada" : "Comparar con: fecha personalizada"}
      >
        <div className="space-y-4">
          <div className="text-sm text-muted">
            Seleccioná un rango y tocá <span className="font-semibold text-text">Aplicar</span>. (UI demo)
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <div className="text-xs font-semibold text-muted">Desde</div>
              <Input className="mt-1" type="date" value={draftFrom} onChange={(e) => setDraftFrom(e.target.value)} />
            </div>
            <div>
              <div className="text-xs font-semibold text-muted">Hasta</div>
              <Input className="mt-1" type="date" value={draftTo} onChange={(e) => setDraftTo(e.target.value)} />
            </div>
          </div>
