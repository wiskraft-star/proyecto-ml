    ...
    <div className="space-y-5">
      <Card>
        ...
        <CardBody className="grid gap-3 md:grid-cols-5">
          <KpiCard
            label="Catálogos monitoreados"
            value={kpis.total}
            format="raw"
            hint="Cantidad de catalog_product_id con oferta propia."
          />
          <KpiCard
            label="Perdiendo (al menos un canal)"
            value={kpis.losingAny}
            format="raw"
            tone={kpis.losingAny ? "bad" : "good"}
            hint="Perdiendo en sin cuotas o con cuotas."
          />
          <KpiCard
            label="Win rate sin cuotas"
            value={kpis.winRateCash}
            format="pct"
            tone={kpis.winRateCash >= 0.8 ? "good" : kpis.winRateCash >= 0.6 ? "warn" : "bad"}
            hint="Proporción ganando el catálogo en cash."
          />
          <KpiCard
            label="Win rate con cuotas"
            value={kpis.winRateCuotas}
            format="pct"
            tone={kpis.winRateCuotas >= 0.8 ? "good" : kpis.winRateCuotas >= 0.6 ? "warn" : "bad"}
            hint="Proporción ganando el catálogo en financiación."
          />
          <KpiCard
            label="Brecha promedio para ganar"
            value={Math.round(visibleCols === "cuotas" ? kpis.avgGapCuotas : kpis.avgGapCash)}
            format="ars"
