...
export function SyncSalesButton({ days = 30 }: { days?: number }) {
  ...
  async function onClick() {
    ...
    setMsg(null);
    try {
      const resp = await fetch("/api/ml/sync-sales", {
        method: "POST",
      ...
      setMsg(`Sync OK. Órdenes: ${json?.fetched_orders ?? "?"}. Ventas: ${json?.sales_upserted ?? "?"}. Errores: ${ecount}.`);
      router.refresh();
    } catch (e: any) {
  ...
  return (
    <div className="row" style={{ justifyContent: "space-between" }}>
      <button className="btn btnPrimary" onClick={onClick} disabled={loading}>
        {loading ? "Sincronizando..." : `Sync Ventas (últimos ${days} días)`}
      </button>
